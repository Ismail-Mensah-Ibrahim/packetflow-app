...
function buildCableLegend(edges: NetworkEdge[]): string {
	...
	if (counts.size === 0)
		return '<tr><td colspan="3" style="color:#64748b;text-align:center">No links</td></tr>';
	return Array.from(counts.entries())
		.map(([type, count]) => {
			const color = (CABLE_COLORS as Record<string, string>)[type] ?? "#94A3B8";
			const vlanList = vlans.get(type);
			const vlanStr = vlanList
				? Array.from(vlanList)
						.map((v) => `V${v}`)
						.join(", ")
				: "—";
...
function buildDeviceTable(nodes: NetworkNode[]): string {
	if (nodes.length === 0)
		return '<tr><td colspan="5" style="color:#64748b;text-align:center">No devices</td></tr>';
	return nodes
		.map(
			(n, i) => `
