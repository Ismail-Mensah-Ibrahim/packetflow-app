	...
	useEffect(() => {
		...
		backendApi
			...
			.then((proj) => {
				...
				setUpdatedAt(
					proj.updated_at ? new Date(proj.updated_at).toLocaleString() : null,
				);
			})
			...
			{!loading && !error && (
				...
				>
					{[
						...
						>
							<Text
								style={{ color: "#F1F5F9", fontSize: 16, fontWeight: "800" }}
							>
								{s.value}
							</Text>
							<Text
			...
			{loading ? (
				...
				<GestureDetector gesture={combined}>
					<Animated.View style={{ flex: 1, overflow: "hidden" }}>
						<Animated.View
							style={[{ width: CANVAS_SIZE, height: CANVAS_SIZE }, canvasStyle]}
						>
							{/* Edges first */}
							{edges.map((e) => (
