	...
	return (
		...
		>
			<View className="flex-row items-center gap-3">
				<View
					className={`w-8 h-8 rounded-full items-center justify-center ${danger ? "bg-destructive/10" : "bg-primary/10"}`}
				>
					{icon}
				</View>
				<Text
					className={`text-base font-medium ${danger ? "text-destructive" : "text-foreground"}`}
				>
					{label}
				</Text>
			</View>
...
export default function ProfileScreen() {
	...
	const menuItems: MenuItem[] = [
		...
		},
		{
			...
			label: "Subscription",
			onPress: () => {},
			right: <SubscriptionBadge tier={tier} />,
		},
	];
	...
	return (
		<View className="flex-1 bg-background">
			...
			>
				...
				{/* Menu */}
				<View
					className="bg-card border border-border rounded-2xl mx-5 overflow-hidden"
					style={{ borderCurve: "continuous" }}
				>
