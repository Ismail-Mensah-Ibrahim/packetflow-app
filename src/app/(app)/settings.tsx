...
function SectionHeader({ title }: { title: string }) {
	return (
		<Text className="text-primary text-xs font-semibold uppercase tracking-wider px-5 pb-2 pt-5">
			{title}
		</Text>
	);
}
	...
	return (
		<View className="flex-1 bg-background">
			...
			>
				...
				<View
					className="bg-card border border-border rounded-2xl mx-4 overflow-hidden"
						...
						right={
							<View className="flex-row gap-1 bg-muted rounded-xl p-1">
								<Pressable
									...
									style={{
										backgroundColor:
											themeMode === "light" ? activeColor : "transparent",
									}}
								>
								...
								<Pressable
									...
									style={{
										backgroundColor:
											themeMode === "dark" ? "#1E293B" : "transparent",
									}}
								>
							...
							<View className="flex-row gap-1">
								{(["metric", "imperial"] as const).map((u) => (
									<Pressable
										...
										style={{
											backgroundColor: units === u ? activeColor : inactiveBg,
										}}
									>
										<Text
											style={{
												color: units === u ? "white" : inactiveText,
												fontSize: 12,
												fontWeight: "500",
											}}
										>
							...
							<View className="flex-row gap-1">
								{(["slow", "normal", "fast"] as const).map((s) => (
									<Pressable
										...
										style={{
											backgroundColor:
												animationSpeed === s ? activeColor : inactiveBg,
										}}
									>
										<Text
											style={{
												color: animationSpeed === s ? "white" : inactiveText,
												fontSize: 11,
												fontWeight: "500",
											}}
										>
					...
					<SettingRow
						...
						right={
							<Switch
								...
								onValueChange={setShowLinkLabels}
								trackColor={{
									true: "#22C55E",
									false: isDark ? "#334155" : "#E5E7EB",
								}}
				...
				>
							...
							<View className="flex-row gap-1">
								{(["supabase", "firebase"] as const).map((b) => (
									<Pressable
										...
										className="px-3 py-1.5 rounded-lg"
										style={{
											backgroundColor: backend === b ? activeColor : inactiveBg,
										}}
									>
										<Text
											style={{
												color: backend === b ? "white" : inactiveText,
												fontSize: 11,
												fontWeight: "600",
											}}
										>
					...
					<SettingRow
						...
						right={
							<Text
								style={{ color: activeColor, fontSize: 12, fontWeight: "700" }}
							>
								{backend === "firebase"
									? "🔥 Firebase Firestore"
									: "⚡ Supabase Postgres"}
				...
				>
					<SettingRow
						label="Version"
						right={<Text className="text-muted-foreground text-sm">1.2.0</Text>}
					/>
					<Divider />
					<SettingRow
						label="Help & Support"
				...
				<View className="mx-4">
					...
					>
						...
						<Text style={{ color: "#EF4444", fontWeight: "700", fontSize: 15 }}>
							{signingOut
								? "Signing out…"
								: `Sign Out (${backend === "firebase" ? "Firebase" : "Supabase"})`}
						</Text>
					</Pressable>
				</View>
