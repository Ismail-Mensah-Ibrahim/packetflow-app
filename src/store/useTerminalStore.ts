...
function getPromptFor(mode: CliMode, hostname: string, _iface: string): string {
	switch (mode) {
		case "user":
			return `${hostname}>`;
		case "privileged":
...
function processIosCommand(
	...
	if (mode === "global_config") {
		...
		if (lparts[0] === "router" && lparts[1] === "bgp") {
			...
			return {
				lines: [
					`% Entering BGP configuration for AS ${asn}.`,
					`% Use 'neighbor', 'network', 'redistribute' commands.`,
				],
			};
		}
		// OSPF process config
		...
		return {
			lines: [`% Unknown config command: '${parts[0]}'. Type '?' for help.`],
		};
	}
	...
	if (mode === "interface_config") {
		if (lparts[0] === "ip" && lparts[1] === "address") {
			...
			return {
				lines: [`% ${currentIface}: IP address ${ip} ${mask} configured.`],
			};
		}
		...
		if (lparts[0] === "no" && lparts[1] === "shutdown") {
			return {
				lines: [
					`%LINK-5-CHANGED: Interface ${currentIface}, changed state to up`,
					`%LINEPROTO-5-UPDOWN: Line protocol on Interface ${currentIface}, changed state to up`,
				],
			};
		}
		if (cmd0 === "shutdown") {
			return {
				lines: [
					`%LINK-5-CHANGED: Interface ${currentIface}, changed state to administratively down`,
					`%LINEPROTO-5-UPDOWN: Line protocol on Interface ${currentIface}, changed state to down`,
				],
			};
		}
		if (cmd0 === "description") {
		...
		if (lparts[0] === "switchport" && lparts[1] === "mode") {
			...
			return {
				lines: [`% ${currentIface}: switchport mode set to ${swMode}.`],
			};
		}
		if (
			lparts[0] === "switchport" &&
		...
		return {
			lines: [`% Unknown interface command: '${parts[0]}'. Type '?' for help.`],
		};
	}
...
export const useTerminalStore = create<TerminalStore>((set, get) => ({
	lines: [
		{
			id: "sys_1",
			text: "PacketFlow IOS Simulator — v12.4",
			type: "system",
			timestamp: new Date(),
		},
		{
			id: "sys_2",
			text: "Type '?' or 'help' for available commands.",
			type: "system",
			timestamp: new Date(),
		},
	...
	executeCommand: (command) => {
		...
		const result = processIosCommand(
			trimmed,
			cliMode,
			hostname,
			currentIface,
	...
	exportLogs: () => {
		const { lines } = get();
		return lines
			.map((l) => `[${l.timestamp.toISOString()}] ${l.text}`)
			.join("\n");
	},
}));
